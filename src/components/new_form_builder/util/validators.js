...
function length(value, length = 0, _, message) {
  if (`${value}`.length == length) return [true];

  return [false, message || `length must be ${length}`.length];
}
...
function alpha(value, limit, _, message) {
  let regex = /^[^0-9!@#$%^&*()_+={}\[\]:;"'<>?,./`~]+$/;
  if (limit instanceof Array) {
    if (limit.length == 1) regex = new RegExp(`^[^0-9!@#$%^&*()_+={}\[\]:;"'<>?,./~]{${limit[0]}}$`);
    if (limit.length > 1)
      regex = new RegExp(`^[^0-9!@#$%^&*()_+={}\[\]:;"'<>?,./~]{${limit[0]}}$`);
  }
...
function num_min(input, limit, _, message) {
  ...
  if (parseInt(input) >= parseFloat(limit)) return [true];
